...
$null = $sb.AppendLine('')
$null = $sb.AppendLine('## 3. URL Base e Ambientes')
$null = $sb.AppendLine('- Local: `http://192.168.15.21:3000`')
...
foreach($pitem in $paths){
  ...
  foreach($mk in $methodOrder){
    ...
    $null = $sb.AppendLine("curl -X '$m' 'http://192.168.15.21:3000$r' -H 'accept: application/json'" + ($(if($auth.req -eq 'sim'){" -H 'Authorization: Bearer <ACCESS_TOKEN>'"}else{''})))
    $null = $sb.AppendLine('```')
