...
function curlExample(method, p, auth) {
  ...
  return `\`\`\`bash\ncurl -X '${m}' \\\n  'http://192.168.15.26:3000${p.replace('{id}', '1').replace('{resource}', 'pessoas-empresas').replace('{relation}', 'ramos').replace('{event}', 'pedido-criado')}' \\\n  -H 'accept: application/json' \\\n${tokenHeader}${ct}${body}\n\`\`\``;
}
...
\`\`\`ts
const res = await fetch('http://192.168.15.26:3000${p.replace('{id}', '1').replace('{resource}', 'pessoas-empresas').replace('{relation}', 'ramos').replace('{event}', 'pedido-criado')}', {
  method: '${m}',
  headers: {
...
- Abrir `http://192.168.15.26:3000/docs`.
...
## 3. URL Base e Ambientes
    - Base local: `http://192.168.15.26:3000`
...
### 7.2 Exemplos
\`\`\`bash
curl "http://192.168.15.26:3000/api/v1/pessoas-empresas?page=1&limit=20&sort=id&order=desc&q=acme"
\`\`\`
\`\`\`bash
curl "http://192.168.15.26:3000/api/v1/produtos-servicos?withDeleted=false&externalSource=erp&externalId=A-100"
\`\`\`
