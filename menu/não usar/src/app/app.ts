...
export async function buildApp(): Promise<FastifyInstance> {
  ...
  await app.register(cors, {
    ...
    origin: (origin, cb) => {
      // Requests server-to-server/no-browser can come without Origin.
      ...
      // Dev convenience: allow localhost/127.0.0.1 and private LAN hosts.
      if (env.NODE_ENV === 'development') {
        try {
          ...
          if (isDev && (isLocalhost || isPrivateClassA || isPrivateClassB || isPrivateClassC)) {
            return cb(null, true);
          }
        } catch {
          // ignore and fallback to deny
        }
      }
  ...
  await app.register(swagger, {
    openapi: {
      info: {
        ...
        description: 'API REST para frontend TAWROS, integrações e automações'
      },
