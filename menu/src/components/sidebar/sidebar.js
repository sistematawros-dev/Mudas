...
export function init() {
  ...
  return () => {
    ...
    if (typeof cleanupQrWarning === 'function') cleanupQrWarning();
    if (typeof cleanupDisabledLinks === 'function') cleanupDisabledLinks();
...
function getUserLicencas() {
  try {
    ...
    return Array.isArray(parsed?.licencas) ? parsed.licencas.map((l) => String(l).toLowerCase()) : [];
  } catch {
    return [];
  }
}

function isTawrosUser() {
  try {
    const raw = sessionStorage.getItem('user');
    if (!raw) return false;
    const parsed = JSON.parse(raw);
    return Number(parsed?.tawros) === 1;
  } catch {
    return false;
  }
}
...
function applyLicencaMenuVisibility() {
  ...
  // Estufa — visível somente com licença 'estufa'
  const estufasToggle = document.querySelector('.sidebar-link[data-toggle="estufas"]');
  const estufasItem = estufasToggle?.closest('.sidebar-item');
  if (estufasItem) {
    estufasItem.style.display = licencas.includes('estufa') ? '' : 'none';
  }

  // Controle de Pátio — visível somente com licença 'patio'
  const patioToggle = document.querySelector('.sidebar-link[data-toggle="controle-patio"]');
  const patioItem = patioToggle?.closest('.sidebar-item');
  if (patioItem) {
    patioItem.style.display = licencas.includes('patio') ? '' : 'none';
  }
}
...
function setupDisabledLinks() {
  const handleDisabledLinkClick = (event) => {
    const disabledLink = event.target?.closest?.('.sidebar-link.is-disabled, .sidebar-link[aria-disabled="true"]');
    if (!disabledLink) return;
    event.preventDefault();
    event.stopPropagation();
  };

  document.addEventListener('click', handleDisabledLinkClick);
  return () => document.removeEventListener('click', handleDisabledLinkClick);
}

function setupQrCodeWarning() {
  const handleQrCodeClick = (event) => {
    const qrCodeLink = event.target?.closest?.('.sidebar-link[data-route="/estufas/ler-qrcode"]');
  ...
  document.addEventListener('click', handleQrCodeClick);
  return () => document.removeEventListener('click', handleQrCodeClick);
}
 ...
 * Usa delegação de eventos no menu para ser mais robusto
 */
...
function updateActiveLink() {
  ...
  links.forEach(link => {
    ...
    const isDisabled = link.classList.contains('is-disabled') || link.getAttribute('aria-disabled') === 'true';
