...
function getPessoasEmpresasContent(state = {}) {
  ...
  const setorOptions = Array.isArray(state.setorOptions) ? state.setorOptions : [];
  ...
  return `
    ...
    const isSelected = String(form.grupo || '') === value ? 'selected' : '';
    ...
    const value = String(item.value || '');
    const label = String(item.label || '');
    const isSelected = String(form.categoria || '') === value ? 'selected' : '';
    return `<option value="${value}" ${isSelected}>${label}</option>`;
  }).join('')}
            </select>
          </div>
            ...
            <div class="cadastro-pessoa-empresa__row cadastro-pessoa-empresa__row--2">
              <div class="cadastro-pessoa-empresa__field">
                ...
                <select id="cpe-setor" class="cadastro-pessoa-empresa__input" data-cpe-field="setor" ${isSaving ? 'disabled' : ''}>
                  ...
                  ${setorOptions.map((item = {}) => {
    const value = String(item.value || '');
    const label = String(item.label || '');
    const isSelected = String(form.setor || '') === value ? 'selected' : '';
    return `<option value="${value}" ${isSelected}>${label}</option>`;
  }).join('')}
                </select>
              </div>
