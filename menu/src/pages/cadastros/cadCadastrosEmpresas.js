...
export function init() {
  ...
  function syncListingVisibility() {
    const shouldHideListing = (currentMode === 'pessoas-empresas' && cadastroState.isOpen)
      || (currentMode === 'produtos-servicos'
        && (cadastroState.isProdutosCadastroOpen || cadastroState.activeProdutosSubTab === 'classificacao'));
  ...
  function openSelecionarClasseDrawer({ triggerEl = null } = {}) {
    ...
    selecionarClasseDrawer.open({
      triggerEl,
      initialSelectedId: classificacao.selectedClasseId || '',
  ...
  function applyMode(mode) {
    ...
    if (!isProdutosServicos && cadastroState.isProdutosCadastroOpen) {
      cadastroState = {
        ...cadastroState,
        isProdutosCadastroOpen: false,
      };
  ...
  const handlePageClick = async (event) => {
    ...
    if (cpsSelectClassButton && currentMode === 'produtos-servicos' && cadastroState.isProdutosCadastroOpen) {
      console.log('Selecionar classe de produto/serviço');
      return;
    }
