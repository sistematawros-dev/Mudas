...
const PEDIDOS_DETAILS_MOCK = {
  'A2W-2025-001': {
    ...
    items: [
      {
        id: 'item-1',
        product: 'Muda de Eucalipto MUD-001',
        quantity: '5.000',
        unitValue: 'R$ 2,50',
  ...
  'A2W-2025-002': {
    ...
    items: [
      ...
      {
        id: 'item-2',
        product: 'Muda de Eucalipto MUD-002',
        quantity: '3.000',
        unitValue: 'R$ 3,00',
...
function initToolbar({ getViewMode = () => KANBAN_VIEW_MODE.KANBAN, setViewMode = () => {} } = {}) {
  ...
  if (visibilityFiltersElement) {
    if (currentKanbanMode === KANBAN_MODE.PEDIDOS) {
      const showFinishedToggle = Toggle.createSimple({
        id: 'kanban-show-finished-toggle',
        size: 'sm',
        checked: ordersListFilters.showFinished,
      }).replace('class="toggle-input"', 'class="toggle-input" data-list-filter="finished"');
      const showCanceledToggle = Toggle.createSimple({
        id: 'kanban-show-canceled-toggle',
        size: 'sm',
        checked: ordersListFilters.showCanceled,
      }).replace('class="toggle-input"', 'class="toggle-input" data-list-filter="canceled"');
      ...
      showFinishedInput = visibilityFiltersElement.querySelector('input[data-list-filter="finished"]');
      showCanceledInput = visibilityFiltersElement.querySelector('input[data-list-filter="canceled"]');
      handleShowFinishedChange = (event) => {
        ordersListFilters.showFinished = Boolean(event.target?.checked);
        saveOrdersListFilters();
        setViewMode(getViewMode());
      };
      handleShowCanceledChange = (event) => {
        ordersListFilters.showCanceled = Boolean(event.target?.checked);
        saveOrdersListFilters();
        setViewMode(getViewMode());
      };
      showFinishedInput?.addEventListener('change', handleShowFinishedChange);
      showCanceledInput?.addEventListener('change', handleShowCanceledChange);
...
function initAgendadoDrawer() {
  ...
  const createStageDrawerClickHandler = ({ stageKey, stageDrawerElement, stageDrawerControls }) => (event) => {
    ...
    if (stageActionButton && stageDrawerElement.contains(stageActionButton)) {
      ...
      if (action === 'consult-agenda' || action === 'consult-location' || action === 'quality' || action === 'qr') {
        console.log(`Ação: ${action}`);
        return;
      }
...
function initPedidoDetailsDrawer() {
  ...
  const renderDrawer = (details) => {
    ...
    footerElement.innerHTML = createPedidoDetailsDrawerFooter();
  };
  ...
  const handleBoardClick = (event) => {
    ...
    if (!cardElement || !boardElement.contains(cardElement)) return;
...
function createPedidoDetailsDrawerContent(details, drawerState) {
  ...
  const tabsHtml = Tabs.createWithPanels({
    ...
    activeTab,
    tabs: [
      { label: 'Informações gerais', content: createPedidoDetailsInfoTab(details) },
...
function createPedidoPlanningMetrics(details, selectedProductId = '') {
  ...
  const metricItems = [
    { label: 'Quantidade Total do Pedido', value: formatPlanningNumber(metrics.total), tone: 'default' },
    { label: 'Quantidade Planejada', value: formatPlanningNumber(metrics.planned), tone: 'primary' },
    { label: 'Quantidade Cancelada', value: formatPlanningNumber(metrics.canceled), tone: 'primary' },
    { label: 'Quantidade Pendente', value: formatPlanningNumber(metrics.pending), tone: 'warning' },
  ];
...
function createPedidoDetailsInfoTab(details) {
  return `
...
function createPedidoItemsCard(items = []) {
  const header = `
      ...
      <article class="orders-item ${hasPlanning && index === 0 ? 'is-expanded' : ''}" data-order-item="${item.id}">
        ...
        ${hasPlanning ? `
          <div class="orders-item__planning">
            ${planRows}
            ${item.planning.some((entry) => entry.type === 'canceled') ? `
...
function createPedidoDetailsDrawerFooter() {
  return `
