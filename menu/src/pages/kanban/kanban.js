...
function applyAgendadoCardInfo(drawerElement, cardInfo) {
  ...
  setAgendadoDetailsValueByLabel(drawerElement, 'Cód. do Pedido', code);
  setAgendadoDetailsValueByLabel(drawerElement, 'Data Agendada do Semeio', plannedDate);
  setAgendadoDetailsValueByLabel(drawerElement, 'Responsável agendamento', line1);
...
function initPedidoDetailsDrawer() {
  ...
  const handleDrawerClick = (event) => {
    ...
    if (toggleButton) {
      ...
      if (!orderItem) return;
      const isCollapsed = orderItem.classList.toggle('is-collapsed');
      toggleButton.setAttribute('aria-expanded', String(!isCollapsed));
      toggleButton.setAttribute('aria-label', isCollapsed ? 'Mostrar item' : 'Ocultar item');
      return;
    }
    ...
    const itemPlanToggleButton = event.target.closest('[data-order-item-plan-toggle]');
    if (itemPlanToggleButton) {
      const planRow = itemPlanToggleButton.closest('[data-order-item-plan-row]');
      if (!planRow) return;
      const isCollapsed = planRow.classList.toggle('is-collapsed');
      itemPlanToggleButton.setAttribute('aria-expanded', String(!isCollapsed));
      itemPlanToggleButton.setAttribute('aria-label', isCollapsed ? 'Expandir detalhes' : 'Recolher detalhes');
      return;
    }
    ...
    const notesToggleButton = event.target.closest('[data-order-summary-notes-toggle]');
    if (notesToggleButton) {
      const notesRoot = notesToggleButton.closest('.orders-summary-notes');
      if (!notesRoot) return;
      const isCollapsed = notesRoot.classList.toggle('is-collapsed');
      notesToggleButton.setAttribute('aria-expanded', String(!isCollapsed));
      notesToggleButton.setAttribute('aria-label', isCollapsed ? 'Expandir observações' : 'Recolher observações');
      return;
    }
...
function getPedidoHistoryEvents(details, selectedProductId = '') {
  ...
  const isReceivedEvent = (eventItem) => {
    const normalizedTitle = normalizePedidoStatusToken(eventItem?.title);
    const normalizedBadge = normalizePedidoStatusToken(eventItem?.badgeType);
    return normalizedTitle === 'pedido recebido' || normalizedBadge === 'inicio';
  };

  const buildAwaitingApprovalEvent = (receivedEvent) => ({
    id: `evt-awaiting-approval-${productId || 'default'}`,
    title: 'Pedido aguardando aprovação',
    date: receivedEvent?.date || '-',
    description: `Pedido ${orderCode || ''} aguardando aprovação do responsável.`,
    badgeLabel: 'Operação',
    badgeType: 'operacao',
    metaRole: 'Responsável',
    metaName: 'Aguardando confirmação',
  });

  const buildInTransitEvent = (referenceEvent) => ({
    id: `evt-in-transit-${productId || 'default'}`,
    title: 'Pedido em trânsito',
    date: referenceEvent?.date || '-',
    description: `Pedido ${orderCode || ''} em trânsito para entrega.`,
    badgeLabel: 'Operação',
    badgeType: 'operacao',
    metaRole: 'Responsável',
    metaName: 'Transporte em andamento',
  });

  const buildFinalizedEvent = (referenceEvent) => ({
    id: `evt-finalized-${productId || 'default'}`,
    title: 'Pedido finalizado',
    date: referenceEvent?.date || '-',
    description: `Pedido ${orderCode || ''} finalizado com entrega concluída.`,
    badgeLabel: 'Sucesso',
    badgeType: 'sucesso',
    metaRole: 'Responsável',
    metaName: 'Entrega concluída',
  });

  if (details?.statusKey === 'received') {
    return events.filter((eventItem) => isReceivedEvent(eventItem));
  }

  if (details?.statusKey === 'awaiting-approval') {
    const receivedEvent = events.find((eventItem) => isReceivedEvent(eventItem));
    const awaitingApprovalEvent = buildAwaitingApprovalEvent(receivedEvent);
    return receivedEvent ? [awaitingApprovalEvent, receivedEvent] : [awaitingApprovalEvent];
  }

  if (details?.statusKey === 'in-preparation') {
    const receivedEvent = events.find((eventItem) => isReceivedEvent(eventItem));
    const awaitingApprovalEvent = buildAwaitingApprovalEvent(receivedEvent);
    ...
    const filteredEvents = events.filter((eventItem) => {
      const normalizedTitle = normalizePedidoStatusToken(eventItem?.title);
      const normalizedBadge = normalizePedidoStatusToken(eventItem?.badgeType);
      const isPlanning = normalizedTitle.startsWith('planejamento realizado') || normalizedBadge === 'planejamento';
      const isCanceled = normalizedTitle === 'item cancelado' || normalizedBadge === 'cancelado';
      return isPlanning || isCanceled || isReceivedEvent(eventItem);
    });

    const baseEvents = filteredEvents.filter((eventItem) => !isReceivedEvent(eventItem));
    if (awaitingApprovalEvent) baseEvents.push(awaitingApprovalEvent);
    if (receivedEvent) baseEvents.push(receivedEvent);
    return baseEvents;
  }

  if (details?.statusKey === 'in-production') {
    return events.filter((eventItem) => {
      const normalizedTitle = normalizePedidoStatusToken(eventItem?.title);
      return !normalizedTitle.startsWith('lote pronto para entrega');
    });
  }

  if (details?.statusKey === 'in-transit') {
    const receivedEvent = events.find((eventItem) => isReceivedEvent(eventItem));
    const referenceEvent = events[0] || receivedEvent;
    const inTransitEvent = buildInTransitEvent(referenceEvent);
    return [inTransitEvent, ...events];
  }

  if (details?.statusKey === 'finalized') {
    const receivedEvent = events.find((eventItem) => isReceivedEvent(eventItem));
    const referenceEvent = events[0] || receivedEvent;
    const inTransitEvent = buildInTransitEvent(referenceEvent);
    const finalizedEvent = buildFinalizedEvent(referenceEvent);
    return [finalizedEvent, inTransitEvent, ...events];
  }
  ...
  return `
    <div class="orders-details-info">
      ...
      <section class="orders-details-section">
        ...
        ${createPedidoItemsCard(details.items, {
          hidePlanCancelButtons: rules?.hidePlanCancelButtons,
          hideReplanButtons: rules?.statusKey === 'in-transit' || rules?.statusKey === 'finalized',
          hideItemPlanningRows: rules?.hideItemPlanningRows,
        })}
      </section>
      <section class="orders-details-section">
...
function createPedidoItemsCard(items = [], options = {}) {
  ...
  const hideReplanButtons = Boolean(options?.hideReplanButtons);
  const hideItemPlanningRows = Boolean(options?.hideItemPlanningRows);
  const header = `
  ...
  const rows = items.map((item, index) => {
    const hasPlanning = !hideItemPlanningRows && Array.isArray(item.planning) && item.planning.length > 0;
    ...
    const planRows = hasPlanning ? item.planning.map((plan, planIndex) => `
      <div class="orders-item-plan-row orders-item-plan-row--${plan.type}" data-order-item-plan-row="${item.id}-${plan.type}-${planIndex}">
...
function createPedidoSummaryCard(summary) {
  const notesText = String(summary?.notes || '-').trim() || '-';
  return `
    <div class="orders-details-card orders-summary-card">
      ...
      <div class="orders-summary-notes">
        ...
        <div class="orders-summary-notes__box">${notesText}</div>
      </div>
    </div>
  `;
}
  ...
  const handleDrawerContentClick = (event) => {
    ...
    if (savedActionButton) {
      ...
      if (isPedidosMode) {
        ...
        if (action === 'delete') {
          const shouldDelete = window.confirm('Confirma a exclusão deste filtro salvo?');
