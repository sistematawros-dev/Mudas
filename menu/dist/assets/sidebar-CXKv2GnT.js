/* empty css                */const v=`\uFEFF<nav class="sidebar" aria-label="Menu principal">
  ...
  <ul class="sidebar-menu" role="list">
      ...
      <ul class="sidebar-submenu" data-submenu="cadastros" role="list">
        ...
        <li class="sidebar-item sidebar-item--has-submenu">
          ...
          <ul class="sidebar-submenu sidebar-submenu--nested" data-submenu="pessoas-empresas" role="list">
            ...
            <li class="sidebar-item">
              <a href="#/cadastros/pessoas-empresas/categoria" class="sidebar-link sidebar-link--nested"
                data-route="/cadastros/pessoas-empresas/categoria">
                <span class="sidebar-text">Categoria</span>
              </a>
            </li>
            <li class="sidebar-item">
              <a href="#/cadastros/pessoas-empresas/setor" class="sidebar-link sidebar-link--nested"
                data-route="/cadastros/pessoas-empresas/setor">
                <span class="sidebar-text">Setor</span>
              </a>
            </li>
            <li class="sidebar-item">
              <a href="#/cadastros/pessoas-empresas/ramo" class="sidebar-link sidebar-link--nested"
                data-route="/cadastros/pessoas-empresas/ramo">
        ...
        <li class="sidebar-item sidebar-item--has-submenu">
          ...
          <ul class="sidebar-submenu sidebar-submenu--nested" data-submenu="produtos-servicos" role="list">            <li class="sidebar-item">
            ...
            <li class="sidebar-item">
              <a href="#/cadastros/produtos-servicos/complementares" class="sidebar-link sidebar-link--nested"
                data-route="/cadastros/produtos-servicos/complementares">
    ...
    <!-- BI e Relatórios -->
    <li class="sidebar-item">
      <a href="#/bi-relatorios" class="sidebar-link" data-route="/bi-relatorios">
    ...
    <li class="sidebar-item sidebar-item--has-submenu">
      ...
      <ul class="sidebar-submenu" data-submenu="estufas" role="list">
        ...
        <li class="sidebar-item">
          <a href="#/estufas/expedicao" class="sidebar-link sidebar-link--sub" data-route="/estufas/expedicao">
            <span class="sidebar-text">Expedição</span>
          </a>
        </li>
        <li class="sidebar-item">
          <a href="#/estufas/operacoes" class="sidebar-link sidebar-link--sub" data-route="/estufas/operacoes">
            <span class="sidebar-text">Operações</span>
          </a>
        </li>
        <li class="sidebar-item">
          <a href="#/estufas/custos" class="sidebar-link sidebar-link--sub" data-route="/estufas/custos">
...
`;function A(e){e.innerHTML=v}function E(){x(),y();const e=C(),s=L();return h(),window.addEventListener("hashchange",h),()=>{window.removeEventListener("hashchange",h),typeof e=="function"&&e(),typeof s=="function"&&s()}}function C(){const e=document.querySelector(".sidebar"),s=document.querySelector(".app-shell"),n="tawros-sidebar-collapsed";if(!e)return;localStorage.getItem(n)==="true"?(e.classList.add("sidebar--collapsed"),s&&s.classList.add("app-shell--sidebar-collapsed")):(e.classList.remove("sidebar--collapsed"),s&&s.classList.remove("app-shell--sidebar-collapsed"));const l=o=>{if(o.target.closest(".header-menu-toggle")){const c=!e.classList.contains("sidebar--collapsed");e.classList.toggle("sidebar--collapsed"),s&&s.classList.toggle("app-shell--sidebar-collapsed"),localStorage.setItem(n,c)}};return document.addEventListener("click",l),()=>document.removeEventListener("click",l)}function M(){const e="tawros-sidebar-collapsed",s=document.querySelector(".sidebar"),n=document.querySelector(".app-shell");if(s){const i=!s.classList.contains("sidebar--collapsed");s.classList.toggle("sidebar--collapsed"),n&&n.classList.toggle("app-shell--sidebar-collapsed"),localStorage.setItem(e,i)}}function x(){document.querySelectorAll("[data-toggle]").forEach(s=>{s.addEventListener("click",()=>{const n=s.closest(".sidebar-item");if(!n)return;const i=n.classList.contains("sidebar-item--expanded");n.classList.toggle("sidebar-item--expanded"),s.setAttribute("aria-expanded",!i)})})}function y(){const e=document.querySelector(".sidebar-search-input");e&&e.addEventListener("input",s=>{const n=s.target.value.toLowerCase().trim(),i=document.querySelectorAll(".sidebar-item");if(!n){i.forEach(l=>{l.style.display="",l.classList.remove("sidebar-item--expanded")});return}i.forEach(l=>{if((l.querySelector(".sidebar-link")?.querySelector(".sidebar-text")?.textContent.toLowerCase()||"").includes(n)){l.style.display="";let b=l.closest(".sidebar-submenu");for(;b;){const p=b.closest(".sidebar-item");p&&(p.classList.add("sidebar-item--expanded"),p.style.display=""),b=p?.parentElement.closest(".sidebar-submenu")}}else l.querySelector('.sidebar-submenu .sidebar-item[style=""]')||(l.style.display="none")})})}function h(){const e=document.querySelectorAll(".sidebar-link[data-route]"),n=(window.location.hash||"#/dashboard").replace("#","");e.forEach(o=>o.classList.remove("active"));let i=null,l=0;if(e.forEach(o=>{const d=o.getAttribute("data-route");if(d&&(n===d||n.startsWith(d+"/"))){const c=d.length;c>l&&(i=o,l=c)}}),i){i.classList.add("active");let o=i.closest(".sidebar-submenu");for(;o;){const d=o.closest(".sidebar-item");if(d){d.classList.add("sidebar-item--expanded");const c=d.querySelector("[data-toggle]");c&&c.setAttribute("aria-expanded","true")}o=d?.parentElement.closest(".sidebar-submenu")}}}function L(){const e=document.querySelector(".sidebar"),s=document.querySelector(".app-shell");if(!e)return()=>{};const n=new Set(Array.from(e.classList).filter(a=>a!=="sidebar--collapsed")),i=e.getAttribute("style")||"",l=new Set(Array.from(s?.classList||[]).filter(a=>a!=="app-shell--sidebar-collapsed")),o=s?.getAttribute("style")||"",d=[".sidebar-header",".sidebar-search",".sidebar-footer",".sidebar-user"],c=new Map;d.forEach(a=>{const r=e.querySelector(a);r&&c.set(a,{className:r.className,style:r.getAttribute("style")||""})});const b=a=>a instanceof Element?!!a.closest(".sidebar-menu"):!1,p=()=>{const a=e.classList.contains("sidebar--collapsed"),r=[...n];a&&r.push("sidebar--collapsed");const t=r.join(" ");e.className!==t&&(e.className=t),(e.getAttribute("style")||"")!==i&&(i?e.setAttribute("style",i):e.removeAttribute("style"))},g=()=>{if(!s)return;const a=s.classList.contains("app-shell--sidebar-collapsed"),r=[...l];a&&r.push("app-shell--sidebar-collapsed");const t=r.join(" ");s.className!==t&&(s.className=t),(s.getAttribute("style")||"")!==o&&(o?s.setAttribute("style",o):s.removeAttribute("style"))},k=a=>{if(a instanceof Element)for(const[r,t]of c.entries()){const u=a.matches(r)?a:a.closest(r);if(!u||b(u))continue;u.className!==t.className&&(u.className=t.className),(u.getAttribute("style")||"")!==t.style&&(t.style?u.setAttribute("style",t.style):u.removeAttribute("style"))}};let m=!1;const f=new MutationObserver(a=>{if(!m){m=!0;try{a.forEach(r=>{if(r.type==="attributes"){const t=r.target;if(t===e){p();return}if(t===s){g();return}b(t)||k(t);return}if(r.type==="childList"){const t=r.target;b(t)||(p(),g())}})}finally{m=!1}}});return f.observe(e,{subtree:!0,childList:!0,attributes:!0,attributeFilter:["class","style"]}),s&&f.observe(s,{subtree:!1,childList:!1,attributes:!0,attributeFilter:["class","style"]}),()=>f.disconnect()}export{E as init,A as render,M as toggleSidebar};
