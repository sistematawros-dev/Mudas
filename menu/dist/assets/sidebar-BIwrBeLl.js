const u=`\uFEFF<nav class="sidebar" aria-label="Menu principal">
  <!-- Header -->
  ...
  <ul class="sidebar-menu" role="list">
    ...
    <li class="sidebar-item sidebar-item--has-submenu">
      ...
      <ul class="sidebar-submenu" data-submenu="estufas" role="list">
        ...
        <li class="sidebar-item">
          <a href="#/pedidos" class="sidebar-link sidebar-link--sub" data-route="/pedidos">
...
`;function h(s){s.innerHTML=u}function g(){p(),m();const s=b();return c(),window.addEventListener("hashchange",c),()=>{window.removeEventListener("hashchange",c),typeof s=="function"&&s()}}function b(){const s=document.querySelector(".sidebar"),e=document.querySelector(".app-shell"),a="tawros-sidebar-collapsed";if(!s)return;localStorage.getItem(a)==="true"?(s.classList.add("sidebar--collapsed"),e&&e.classList.add("app-shell--sidebar-collapsed")):(s.classList.remove("sidebar--collapsed"),e&&e.classList.remove("app-shell--sidebar-collapsed"));const n=r=>{if(r.target.closest(".header-menu-toggle")){const o=!s.classList.contains("sidebar--collapsed");s.classList.toggle("sidebar--collapsed"),e&&e.classList.toggle("app-shell--sidebar-collapsed"),localStorage.setItem(a,o)}};return document.addEventListener("click",n),()=>document.removeEventListener("click",n)}function f(){const s="tawros-sidebar-collapsed",e=document.querySelector(".sidebar"),a=document.querySelector(".app-shell");if(e){const t=!e.classList.contains("sidebar--collapsed");e.classList.toggle("sidebar--collapsed"),a&&a.classList.toggle("app-shell--sidebar-collapsed"),localStorage.setItem(s,t)}}function p(){document.querySelectorAll("[data-toggle]").forEach(e=>{e.addEventListener("click",()=>{const a=e.closest(".sidebar-item");if(!a)return;const t=a.classList.contains("sidebar-item--expanded");a.classList.toggle("sidebar-item--expanded"),e.setAttribute("aria-expanded",!t)})})}function m(){const s=document.querySelector(".sidebar-search-input");s&&s.addEventListener("input",e=>{const a=e.target.value.toLowerCase().trim(),t=document.querySelectorAll(".sidebar-item");if(!a){t.forEach(n=>{n.style.display="",n.classList.remove("sidebar-item--expanded")});return}t.forEach(n=>{if((n.querySelector(".sidebar-link")?.querySelector(".sidebar-text")?.textContent.toLowerCase()||"").includes(a)){n.style.display="";let l=n.closest(".sidebar-submenu");for(;l;){const d=l.closest(".sidebar-item");d&&(d.classList.add("sidebar-item--expanded"),d.style.display=""),l=d?.parentElement.closest(".sidebar-submenu")}}else n.querySelector('.sidebar-submenu .sidebar-item[style=""]')||(n.style.display="none")})})}function c(){const s=document.querySelectorAll(".sidebar-link[data-route]"),a=(window.location.hash||"#/dashboard").replace("#","");s.forEach(r=>r.classList.remove("active"));let t=null,n=0;if(s.forEach(r=>{const i=r.getAttribute("data-route");if(i&&(a===i||a.startsWith(i+"/"))){const o=i.length;o>n&&(t=r,n=o)}}),t){t.classList.add("active");let r=t.closest(".sidebar-submenu");for(;r;){const i=r.closest(".sidebar-item");if(i){i.classList.add("sidebar-item--expanded");const o=i.querySelector("[data-toggle]");o&&o.setAttribute("aria-expanded","true")}r=i?.parentElement.closest(".sidebar-submenu")}}}export{g as init,h as render,f as toggleSidebar};
